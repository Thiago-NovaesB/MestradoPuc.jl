...
# Ler dados 
# ============================================================
data = CSV.read("Consumo_Res_Sul_Apenas a Serie.csv", DataFrame);
y = data[!,"data"]
n = length(y)
...
res = optimize(get_minus_log_likelihood, [sqrt(var(y)), sqrt(var(y)), 1.0, 1.0], LBFGS(),Optim.Options(show_trace=true)) #NelderMead
...
# Y
plot(y, label = "Real", legend=:bottomright)
